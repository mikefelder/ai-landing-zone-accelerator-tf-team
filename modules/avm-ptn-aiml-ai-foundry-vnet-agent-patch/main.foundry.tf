resource "azapi_resource" "ai_foundry" {
  ...
  body = {
    ...
    properties = {
      ...
      publicNetworkAccess    = coalesce(var.ai_foundry.public_network_access_enabled, !var.create_private_endpoints) ? "Enabled" : "Disabled"
      networkAcls = {
        defaultAction       = "Allow"
        virtualNetworkRules = []
        ipRules             = []
...
resource "azapi_resource" "ai_agent_capability_host" {
  ...
  body = {
    properties = merge({
      ...
      }, var.ai_foundry.network_injections != null ? {
      customerSubnet = var.ai_foundry.network_injections[0].subnetArmId
    } : {})
  }
  create_headers            = var.enable_telemetry ? { "User-Agent" : local.avm_azapi_header } : null
  delete_headers            = var.enable_telemetry ? { "User-Agent" : local.avm_azapi_header } : null
